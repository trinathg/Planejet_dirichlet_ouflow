...
{	
	...
	tdma1x(fvar,0);  // Computing du/dx required for the outflow pressure boundary condition
	...
	for(int j=0;j<=ny;j++)
	{
		...
		//fvar[ind].u[0] = Re*( fvar[ind].u[2] + (1./4.)*( (fvar[ind].u[5]*fvar[ind].u[5]) + (fvar[ind].u[6]*fvar[ind].u[6]) )*(1. - tanh(fvar[ind].u[5]/delta)) - RRe*(fvar[ind].ux[5] + fvar[ind].uy[6]) )*(-1./B1)*dx - B2*fvar[ind-1].u[0] - B3*fvar[ind-2].u[0] - B4*fvar[ind-3].u[0];
		...
		fvar[ind].u[2] = (1./Re)*fvar[ind].ux[0] - (1./4.)*( (fvar[ind].u[0]*fvar[ind].u[0]) + (fvar[ind].u[1]*fvar[ind].u[1]) )*(1. - tanh(fvar[ind].u[0]/delta)); //Dirichlet boundary condition from the paper by Dong et al
	}	

...
{  
	...
	for(int j=0;j<=ny;j++)
	{
		...
		{   
			...
			//fvar[ind].u[0] = 1.0/(1.+sinh(node[ind].x[1]));
			//fvar[ind].u[1] = 0.0; 
	...
	{	
		...
		for(int j=0;j<=ny;j=j+sp)
		{
	  		for(int i=0;i<=nx;i=i+sp)
	  		{
				...
				//level[lev].F[ind] = -2.*M_PI*M_PI*sin(M_PI*i*dx)*cos(M_PI*j*dy);  //For phi = sin(pi*x)cos(pi*x)
				//level[lev].F[ind] = -2.*M_PI*M_PI*cos(M_PI*i*dx)*cos(M_PI*j*dy);  //For phi = cos(pi*x)cos(pi*y)
				//level[lev].F[ind] = -M_PI*M_PI*sin(M_PI*i*dx);
				//level[lev].F[ind] = -M_PI*M_PI*cos(M_PI*j*dy);				
				level[lev].F[ind] = 0.0;
